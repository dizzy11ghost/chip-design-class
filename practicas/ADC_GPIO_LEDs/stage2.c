...
int main(void) {
    ...
    // Creación de la Cola para 10 mensajes
    ...
    // Creación de tareas (Nombres de funciones corregidos aquí)
...
static void vTaskLightSensor(void *pvParameters)
{
	sensor_msg_t msg; // <-- Corregido: Agregada variable local faltante
...
static void vTaskTemperatureSensor(void *pvParameters) // <-- Corregido: Nombre de la función alineado con el prototipo
{
	sensor_msg_t msg;
...
static void vTaskLedControl(void *pvParameters)
{
	...
	while(1)
	{
		// Bloqueo eficiente hasta recibir un mensaje de la cola
