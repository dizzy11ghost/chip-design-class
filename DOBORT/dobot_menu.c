...
int main(void)
{
    ...
    while (1) {
        ...
        handle_key(key); //para ignorar todo lo que no sea A, B o C
    }
}
...
void show_menu(void)
{
    ...
    delayMs(2);
    LCD_command(0x80);
    LCD_string("A: Acomodar");
    LCD_command(0xC0);
    LCD_string("B: Recibir C: Salir");
...
void handle_key(char key)
{
    ...
    switch (key) {
        case 'C': color_name = "RED";   led_value = 1; break;
        case 'B': color_name = "BLUE";  led_value = 4; break;
        case 'A': color_name = "GREEN"; led_value = 2; break;
        default:  return;
    }
...
void LED_set(int value)
{
    if (value & 1) PTB->PCOR = 0x40000 ; else PTB->PSOR = 0x40000; //magenta
    if (value & 1) PTD->PCOR = 0x02; else PTD->PCOR = 0x02; //magenta
    if (value & 2) PTB->PCOR = 0x80000; else PTB->PSOR = 0x80000; //naranja
    if (value & 2) PTB->PCOR = 0x40000; else PTB->PSOR = 0x40000; //naranja
